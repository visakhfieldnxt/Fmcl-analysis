...
//                             debPayAcct: results[i].DebPayAcct,
//                             glblLocNum: results[i].GlblLocNum,
...
//                             cardCode: itemResult[i].CardCode,
//                             createdAt: new Date(),
//                             uuid: Random.id()
//                         });
//                     } else {
...
// // // // start Supplier portal crons
// // // start Supplier Order cron
// SyncedCron.add({
//     name: 'Supplier Order Cron',
//     schedule: function (parser) {
//         // parser is a later.parse object
//         return parser.text('every 5 min');
//     },
//     job: function () {
//         let userGet = allUsers.find({ cardType: "Supplier" }, { fields: { cardCode: 1 } }).fetch();
//         let userArray = [];
//         let userArrayUnique = [];
//         let resendArray = [];
//         let resendArrayUnique = [];
//         //for getting supplier user
//         for (let i = 0; i < userGet.length; i++) {
//             userArray.push(userGet[i].cardCode);
//         }
//         //for getting unique cardCode from array
//         userArrayUnique = userArray.filter(function (itm, i, userArray) {
//             return i == userArray.indexOf(itm);
//         });
//         //for getting resend cardcode
//         for (let r = 0; r < userArrayUnique.length; r++) {
//             let customerErrorfind = CronResult.find({
//                 cardCode: userArrayUnique[r],
//                 name: 'Supplier Purchase Order Cron',
//                 cronResult: false
//             }).fetch();
//             if (customerErrorfind.length !== undefined && customerErrorfind.length !== 0) {
//                 for (let i = 0; i < customerErrorfind.length; i++) {
//                     resendArray.push(customerErrorfind[i].cardCode);
//                 }
//             }

//         }
//         //for getting unique resend cardcode from Array
//         resendArrayUnique = resendArray.filter(function (itm, i, resendArray) {
//             return i == resendArray.indexOf(itm);
//         });
//         console.log("resendArray", resendArray);
//         console.log("resendArrayUnique", resendArrayUnique);

//         //formatting cardcodes with single quotes and comma
//         let cardCodes = resendArrayUnique.map(x => "'" + x + "'").toString()
...
//         if (resendArrayUnique.length > 0) {
//             console.log("supplier order http call success");
//             HTTP.call("POST", url, options, (err, result) => {
//                 if (err) {
//                     // response = err;
...
//     }
// });
// //   // Supplier Order Item Cron
// SyncedCron.add({
//     name: 'Supplier Order Item Cron',
//     schedule: function (parser) {
//         // parser is a later.parse object
//         return parser.text('every 5 min');
//     },
//     job: function () {
//         let userGet = allUsers.find({ cardType: "Supplier" }).fetch();
//         let userArray = [];
//         let userArrayUnique = [];
//         let resendArray = [];
//         let resendArrayUnique = [];
...
//         let updateCount = 0;
//         if (resendArrayUnique.length > 0) {
//             console.log("supplier order item http call success");
//             HTTP.call("POST", url, options, (err, result) => {
//                 if (err) {
//                     console.log("Supplier Order Item Cron", err);
//                     return err;
//                 } else if (result.data !== null && result.data !== undefined) {
//                     let orderResult = result.data.data;
//                     // console.log("Supplier Order Item Cron", orderResult);
//                     for (let i = 0; i < orderResult.length; i++) {
//                         let orderData = SupplierOrder.find({ docEntry: orderResult[i].DocEntry }).fetch();
...
//                                     if (orderItems[l].itemCode === orderResult[i].ItemCode) {
//                                         orderItems[l].docEntry = orderResult[i].DocEntry,
//                                             orderItems[l].lineNum = orderResult[i].LineNum,
//                                             orderItems[l].docEntry = orderResult[i].DocEntry,
//                                             orderItems[l].dscription = orderResult[i].Dscription,
//                                             orderItems[l].quantity = orderResult[i].Quantity,
//                                             orderItems[l].price = orderResult[i].Price,
//                                             orderItems[l].discPrcnt = orderResult[i].DiscPrcnt,
//                                             orderItems[l].grossTotal = orderResult[i].LineTotal,
//                                             orderItems[l].whsCode = orderResult[i].WhsCode,
//                                             orderItems[l].taxStatus = orderResult[i].TaxStatus,
//                                             orderItems[l].useBaseUn = orderResult[i].UseBaseUn,
//                                             orderItems[l].uomEntry = orderResult[i].UomEntry,
//                                             orderItems[l].uomCode = orderResult[i].UomCode,
//                                             orderItems[l].u_PrcType = orderResult[i].U_PrcType,
//                                             orderItems[l].u_TaxAmt = orderResult[i].U_TaxAmt,
//                                             orderItems[l].freeTxt = orderResult[i].FreeTxt,
//                                             orderItems[l].vatGroup = orderResult[i].VatGroup,
//                                             orderItems[l].vatSum = orderResult[i].VatSum,
//                                             orderItems[l].inclusivePrice = orderResult[i].InclusivePrice,
//                                             orderItems[l].exclusivePrice = orderResult[i].ExclusivePrice,
//                                             orderItems[l].weightItem = orderResult[i].Weight,
//                                             orderItems[l].unitDisplyItem = orderResult[i].UnitDisply,
//                                             orderItems[l].updatedAt = new Date();
//                                     }
//                                 }
//                                 let entry = orderItems.find(function (e) { return e.itemCode === orderResult[i].ItemCode; });
//                                 // console.log("entryVal", entry);
//                                 if (!entry) {
//                                     let itemData = {
//                                         docEntry: orderResult[i].DocEntry,
//                                         lineNum: orderResult[i].LineNum,
//                                         itemCode: orderResult[i].ItemCode,
//                                         dscription: orderResult[i].Dscription,
//                                         quantity: orderResult[i].Quantity,
//                                         price: orderResult[i].Price,
//                                         discPrcnt: orderResult[i].DiscPrcnt,
//                                         grossTotal: orderResult[i].LineTotal,
//                                         whsCode: orderResult[i].WhsCode,
//                                         taxStatus: orderResult[i].TaxStatus,
//                                         useBaseUn: orderResult[i].UseBaseUn,
//                                         uomEntry: orderResult[i].UomEntry,
//                                         uomCode: orderResult[i].UomCode,
//                                         u_PrcType: orderResult[i].U_PrcType,
//                                         u_TaxAmt: orderResult[i].U_TaxAmt,
//                                         freeTxt: orderResult[i].FreeTxt,
//                                         vatGroup: orderResult[i].VatGroup,
//                                         vatSum: orderResult[i].VatSum,
//                                         inclusivePrice: orderResult[i].InclusivePrice,
//                                         exclusivePrice: orderResult[i].ExclusivePrice,
//                                         weightItem: orderResult[i].Weight,
//                                         unitDisplyItem: orderResult[i].UnitDisply,
//                                         updatedAt: new Date()
//                                     }
...
// // start Supplier Invoice cron
// SyncedCron.add({
//     name: 'Supplier Invoice Cron',
//     schedule: function (parser) {
//         // parser is a later.parse object
//         return parser.text('every 5 min');
//     },
...
//         let insertCount = 0;
//         let updateCount = 0;
//         console.log("dataArray supplier invoice", dataArray);
//         if (resendArrayUnique.length > 0) {
//             console.log("supplier invoice http call success");
//             HTTP.call("POST", url, options, (err, result) => {
//                 if (err) {
//                     return err;
//                 } else if (result.data !== null && result.data !== undefined) {
...
//                             docEntry: orderResult[i].DocEntry
//                         }).fetch();
//                         // console.log("invoiceFind", invoiceFind);
...
//                                 docTotal: orderResult[i].DocTotal,
//                                 comments: orderResult[i].Comments,
//                                 paidToDate: orderResult[i].PaidToDate,
//                                 weight: orderResult[i].Weight,
//                                 unitDisply: orderResult[i].UnitDisply,
//                                 priceMode: orderResult[i].PriceMode,
//                                 slpName: orderResult[i].SlpName,
...
//     }
// });
// //   // Supplier Invoice Item Cron 
// SyncedCron.add({
//     name: 'Supplier Invoice Item Cron',
//     schedule: function (parser) {
//         // parser is a later.parse object
//         return parser.text('every 5 min');
//     },
...
//         if (resendArrayUnique.length > 0) {
//             console.log("supplier invoice item http call success");
//             HTTP.call("POST", url, options, (err, result) => {
//                 if (err) {
//                     console.log("Supplier Invoice Item Cron err", err);
//                     return err;
//                 } else if (result.data !== null && result.data !== undefined) {
//                     let orderResult = result.data.data;
//                     // console.log("Supplier Invoice Item Cron", orderResult);
//                     for (let i = 0; i < orderResult.length; i++) {
//                         let orderData = SupplierInvoice.find({ docEntry: orderResult[i].DocEntry }).fetch();
//                         if (orderData.length > 0) {
//                             let orderItems = [];
//                             orderItems = orderData[0].itemLines;
//                             if (orderItems !== '' && orderItems !== undefined) {
...
//                                             orderItems[l].u_PrcType = orderResult[i].U_PrcType,
//                                             orderItems[l].u_TaxAmt = orderResult[i].U_TaxAmt,
//                                             orderItems[l].u_GrossAmt = orderResult[i].U_GrossAmt,
//                                             orderItems[l].vatSum = orderResult[i].VatSum,
//                                             orderItems[l].inclusivePrice = orderResult[i].InclusivePrice,
//                                             orderItems[l].exclusivePrice = orderResult[i].ExclusivePrice,
//                                             orderItems[l].weightItem = orderResult[i].Weight,
//                                             orderItems[l].unitDisplyItem = orderResult[i].UnitDisply,
//                                             orderItems[l].baseRef = orderResult[i].BaseRef,
//                                             orderItems[l].baseType = orderResult[i].BaseType,
//                                             orderItems[l].vatGroup = orderResult[i].VatGroup,
//                                             orderItems[l].updatedAt = new Date();
//                                     }
//                                 }
...
//                                         u_PrcType: orderResult[i].U_PrcType,
//                                         u_TaxAmt: orderResult[i].U_TaxAmt,
//                                         u_GrossAmt: orderResult[i].U_GrossAmt,
//                                         vatSum: orderResult[i].VatSum,
//                                         inclusivePrice: orderResult[i].InclusivePrice,
//                                         exclusivePrice: orderResult[i].ExclusivePrice,
//                                         weightItem: orderResult[i].Weight,
//                                         unitDisplyItem: orderResult[i].UnitDisply,
//                                         baseRef: orderResult[i].BaseRef,
//                                         baseType: orderResult[i].BaseType,
//                                         vatGroup: orderResult[i].VatGroup,
//                                         updatedAt: new Date()
//                                     }
//                                     orderItems.push(itemData);
...
//                                 totalQty += Number(itemsQty[i].quantity);
//                             }
//                             SupplierInvoice.update(orderData[0]._id, {
//                                 $set: {
//                                     itemLines: orderItems,
//                                     totalQty: totalQty.toString(),
//                                     totalItem: itemsQty.length.toString(),
...
//     }
// });
// //End Supplier Invoice Item Cron 

// // Start Updated Supplier Order cron
// SyncedCron.add({
//     name: 'Updated Supplier Order Cron',
//     schedule: function (parser) {
//         // parser is a later.parse object
//         return parser.text('every 5 min');
//     },
...
//         });
//         //formatting cardcodes with single quotes and comma
//         let cardCodes = userArrayUnique.map(x => "'" + x + "'").toString()
//         console.log("cardCodes", cardCodes);
//         let base_url = Config.findOne({
//             name: 'base_url'
//         }).value;
...
//             name: 'dbId'
//         }).value;
//         let url = base_url + updatedSupplierOrderDataGet_Url;
//         let date = moment(new Date()).format("YYYY.MM.DD");
//         let fullDate = date + " 00:00:00.000";
//         let dataArray = {
//             dbId: dbId,
...
//     }
// });
// // End Updated Supplier Order cron

// //   // Start Updated Supplier Order Item Cron
// SyncedCron.add({
//     name: 'Updated Supplier Order Item Cron',
//     schedule: function (parser) {
//         // parser is a later.parse object
//         return parser.text('every 5 min');
//     },
...
//             name: 'dbId'
//         }).value;
//         let url = base_url + updatedSupplierOrderItemDataGet_Url;
//         let date = moment(new Date()).format("YYYY.MM.DD");
//         let fullDate = date + " 00:00:00.000";
//         let dataArray = {
//             dbId: dbId,
...
//         HTTP.call("POST", url, options, (err, result) => {
//             if (err) {
//                 console.log("Updated Supplier Order Item Cron", err);
//                 return err;
//             } else if (result.data !== null && result.data !== undefined) {
//                 let orderResult = result.data.data;
//                 console.log("Updated Supplier Order Item Cron", orderResult);
//                 for (let i = 0; i < orderResult.length; i++) {
//                     let orderData = SupplierOrder.find({ docEntry: orderResult[i].DocEntry }).fetch();
//                     if (orderData.length > 0) {
//                         let orderItems = [];
...
//     }
// });
// //End Updated Supplier Order Item Cron

// // Start Updated Supplier Invoice cron
// SyncedCron.add({
//     name: 'Updated Supplier Invoice cron',
//     schedule: function (parser) {
//         // parser is a later.parse object
//         return parser.text('every 5 min');
//     },
...
//             name: 'dbId'
//         }).value;
//         let url = base_url + updatedSupplierInvoiceGet_Url;
//         let date = moment(new Date()).format("YYYY.MM.DD");
//         let fullDate = date + " 00:00:00.000";
//         let dataArray = {
//             dbId: dbId,
...
//     }
// });
// // End Updated Supplier Invoice cron

// //   // Start Updated Supplier Invoice Item Cron  
// SyncedCron.add({
//     name: 'Updated Supplier Invoice Item Cron ',
//     schedule: function (parser) {
//         // parser is a later.parse object
//         return parser.text('every 5 min');
//     },
...
//             name: 'dbId'
//         }).value;
//         let url = base_url + updatedSupplierInvoiceItemGet_Url;
//         let date = moment(new Date()).format("YYYY.MM.DD");
//         let fullDate = date + " 00:00:00.000";
//         let dataArray = {
//             dbId: dbId,
...
//         HTTP.call("POST", url, options, (err, result) => {
//             if (err) {
//                 console.log("Updated Supplier Invoice Item Cron  err", err);
//                 return err;
//             } else if (result.data !== null && result.data !== undefined) {
//                 let orderResult = result.data.data;
//                 console.log("Updated Supplier Invoice Item Cron ", orderResult);
//                 for (let i = 0; i < orderResult.length; i++) {
//                     let orderData = SupplierInvoice.find({ docEntry: orderResult[i].DocEntry }).fetch();
//                     if (orderData.length > 0) {
//                         let orderItems = [];
...
//     }
// });
// //End Updated Supplier Invoice Item Cron





// // start Customer Invoice cron
// SyncedCron.add({
//     name: 'Customer Portal Invoice Cron',
//     schedule: function (parser) {
//         // parser is a later.parse object
//         return parser.text('every 5 min');
//     },
//     job: function () {
//         let userGet = allUsers.find({ cardType: "Customer" }).fetch();
//         let userArray = [];
//         let userArrayUnique = [];
//         let resendArray = [];
//         let resendArrayUnique = [];
...
//         let updateCount = 0;
//         if (resendArrayUnique.length > 0) {
//             console.log("Customer Invoice Cron http call success");
//             HTTP.call("POST", url, options, (err, result) => {
//                 if (err) {
//                     return err;
//                 } else if (result.data !== null && result.data !== undefined) {
...
//                                 userId: '',
//                                 docDueDate: orderResult[i].DocDueDate,
//                                 dueDate: orderResult[i].DocDueDate,
//                                 docStatus: orderResult[i].DocStatus,
//                                 docEntry: orderResult[i].DocEntry,
//                                 canceled: orderResult[i].CANCELED,
//                                 docDate: new Date(moment(orderResult[i].DocDate).format('YYYY-MM-DD')),
//                                 docDateIso: new Date(orderResult[i].DocDate),
//                                 beforeDiscount: orderResult[i].DocTotal,
//                                 afterDiscount: orderResult[i].DocTotal,
//                                 GST: orderResult[i].VatSum,
//                                 discountPercentage: orderResult[i].DiscPrcnt,
//                                 remark_order: orderResult[i].Comments,
//                                 docTotal: orderResult[i].DocTotal,
//                                 grandTotal: orderResult[i].DocTotal,
//                                 docNum: orderResult[i].DocNum,
//                                 currency: '',
//                                 transporterName: '',
//                                 vehicleNoAssignee: '',
...
// //   // Customer Invoice Item Cron 
// SyncedCron.add({
//     name: 'Customer Portal Invoice Item Cron',
//     schedule: function (parser) {
//         // parser is a later.parse object
//         return parser.text('every 5 min');
//     },
...
//             name: 'dbId'
//         }).value;
//         let url = base_url + customerInvoiceItemGet_Url;
//         let dataArray = {
//             dbId: dbId,
//             cardCodes: cardCodes
//         }
...
//         console.log("dataArrayinvoiceItem", dataArray);
//         if (resendArrayUnique.length > 0) {
//             console.log("Customer Invoice Item Cron  http call success");
//             HTTP.call("POST", url, options, (err, result) => {
//                 if (err) {
//                     console.log("Customer Invoice Item Cron err", err);
//                     return err;
//                 } else if (result.data !== null && result.data !== undefined) {
//                     let orderResult = result.data.data;
//                     // console.log("Supplier Invoice Item Cron", orderResult);
//                     for (let i = 0; i < orderResult.length; i++) {
//                         let orderData = Invoice.find({ docEntry: orderResult[i].DocEntry }).fetch();
//                         if (orderData.length > 0) {
//                             let orderItems = [];
//                             orderItems = orderData[0].itemLines;
//                             if (orderItems !== '' && orderItems !== undefined) {
...
//                                     if (orderItems[l].itemCode === orderResult[i].ItemCode) {
//                                         orderItems[l].docEntry = orderResult[i].DocEntry,
//                                             orderItems[l].baseLine = orderResult[i].LineNum,
//                                             orderItems[l].docEntry = orderResult[i].DocEntry,
//                                             orderItems[l].itemNam = orderResult[i].Dscription,
//                                             orderItems[l].quantity = orderResult[i].Quantity,
//                                             orderItems[l].price = orderResult[i].Price,
//                                             orderItems[l].unitPrice = orderResult[i].Price,
//                                             orderItems[l].salesPrice = orderResult[i].Price,
//                                             orderItems[l].incPrice = orderResult[i].Price,
//                                             orderItems[l].excPrice = orderResult[i].Price,
//                                             orderItems[l].vatGroup = orderResult[i].VatGroup,
...
//                                             orderItems[l].uomCode = orderResult[i].UomCode,
//                                             orderItems[l].u_PrcType = orderResult[i].U_PrcType,
//                                             orderItems[l].taxRate = orderResult[i].U_TaxAmt,
//                                             orderItems[l].u_GrossAmt = orderResult[i].U_GrossAmt,
//                                             orderItems[l].vatSum = orderResult[i].VatSum,
//                                             orderItems[l].invWeight = orderResult[i].Weight,
//                                             orderItems[l].unitDisplyItem = orderResult[i].UnitDisply,
//                                             orderItems[l].baseRef = orderResult[i].BaseRef,
//                                             orderItems[l].baseType = orderResult[i].BaseType,
//                                             orderItems[l].updatedAt = new Date();
...
//                                     let itemData = {
//                                         docEntry: orderResult[i].DocEntry,
//                                         baseLine: orderResult[i].LineNum,
//                                         itemCode: orderResult[i].ItemCode,
//                                         itemNam: orderResult[i].Dscription,
//                                         quantity: orderResult[i].Quantity,
//                                         price: orderResult[i].Price,
//                                         salesPrice: orderResult[i].Price,
//                                         unitPrice: orderResult[i].Price,
//                                         incPrice: orderResult[i].Price,
//                                         excPrice: orderResult[i].Price,
//                                         discPrcnt: orderResult[i].DiscPrcnt,
//                                         grossTotal: orderResult[i].LineTotal,
//                                         whsCode: orderResult[i].WhsCode,
//                                         taxStatus: orderResult[i].TaxStatus,
...
//                                         uomCode: orderResult[i].UomCode,
//                                         u_PrcType: orderResult[i].U_PrcType,
//                                         taxRate: orderResult[i].U_TaxAmt,
//                                         u_GrossAmt: orderResult[i].U_GrossAmt,
//                                         vatSum: orderResult[i].VatSum,
//                                         invWeight: orderResult[i].Weight,
//                                         unitDisplyItem: orderResult[i].UnitDisply,
//                                         baseRef: orderResult[i].BaseRef,
//                                         baseType: orderResult[i].BaseType,
//                                         vatGroup: orderResult[i].VatGroup,
...
//                                 totalQty += Number(itemsQty[i].quantity);
//                             }
//                             Invoice.update(orderData[0]._id, {
//                                 $set: {
//                                     itemLines: orderItems,
//                                     totalQty: totalQty.toString(),
//                                     totalItem: itemsQty.length.toString(),
...
//     }
// });
// //End Customer Invoice Item Cron



// // Start Updated Customer Invoice cron
// SyncedCron.add({
//     name: 'Updated Customer Invoice cron',
//     schedule: function (parser) {
//         // parser is a later.parse object
//         return parser.text('every 5 min');
//     },
...
// //   // Start Updated Customer Invoice Item Cron  
// SyncedCron.add({
//     name: 'Updated Customer Invoice Item Cron ',
//     schedule: function (parser) {
//         // parser is a later.parse object
//         return parser.text('every 5 min');
//     },
...
//             name: 'dbId'
//         }).value;
//         let url = base_url + updatedCustomerInvoiceItemGet_Url;
//         let date = moment(new Date()).format("YYYY.MM.DD");
//         let fullDate = date + " 00:00:00.000";
//         let dataArray = {
//             dbId: dbId,
...
//         HTTP.call("POST", url, options, (err, result) => {
//             if (err) {
//                 console.log("Updated Customer Invoice Item Cron  err", err);
//                 return err;
//             } else if (result.data !== null && result.data !== undefined) {
//                 let orderResult = result.data.data;
//                 console.log("Updated Customer Invoice Item Cron ", orderResult);
//                 for (let i = 0; i < orderResult.length; i++) {
//                     let orderData = Invoice.find({ docEntry: orderResult[i].DocEntry }).fetch();
//                     if (orderData.length > 0) {
//                         let orderItems = [];
