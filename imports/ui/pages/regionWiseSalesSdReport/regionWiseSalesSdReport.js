 ...
 */
 ...
 Template.regionWiseSalesSdReport.onCreated(function () {
   let self = this;
   self.autorun(() => {
 ...
 Template.regionWiseSalesSdReport.onRendered(function () {
    ...
    $('.regionfilter').select2({
      ...
      Meteor.call('creditSale.regionalWiseSaleSdList', fromDate, toDate, verticalName='', regionName='', (err, res) => {
          if (!err) {
 ...
 Template.regionWiseSalesSdReport.helpers({
 ...
 Template.regionWiseSalesSdReport.events({
    'click #filterSearch': () => {
    ...
    'submit .verticalwise-filter' : (event, template) => {
        ...
        let regionName = event.target.regionfilter.value;
        let first = $("#fromDate").val();
        let second = $("#toDate").val();   
        if(regionName==''){
          setTimeout(function () {
            $("#regionNameArrayspan1").html('<style> #regionNameArrayspan { color:#fc5f5f }</style><span id ="regionNameArrayspan">Please Select Region</span>').fadeIn('fast');
          }, 0);
          return;
        }
         if(verticalName==''){
