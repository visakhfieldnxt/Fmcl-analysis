...
let latitude = '';
let longitude = '';
Template.createCreditSale.onCreated(function () {
 ...
 */
  $('.salesType').select2({
    ...
    tokenSeparators: [','],
    allowClear: true,
    dropdownParent: $(".salesType").parent(),
  });
  /**
* TODO: Complete JS doc
*/
...
Template.createCreditSale.helpers({
  /**
   * get product list
   */
  getProductList: () => {
...
let itemArray = [];
let baseQuantity = '';
let walkInCustomer = false;
Template.createCreditSale.events({
  /**
    * TODO: Complete JS doc
   */
  'click .closestocks': (event, template) => {
    ...
    $("#salesType").val('').trigger('change');
    FlowRouter.go('creditSale');
    template.itemArray.set('');
    itemArray = [];
    $('.amtShowDiv').css('display', 'none');
    $(".selectVertical").prop("disabled", false);
    $("#addCashSales").val('').trigger('change');
    $('#salesType').prop('disabled', false);
  ...
  'submit .addCashSales': (event, template) => {
    ...
    let salesType = '';
    $('#salesType').find(':selected').each(function () {
      salesType = $(this).val();
    });
    ...
    let creditAmt = Template.instance().creditAmtVal.get();
    if (productsArray.length === 0) {
      toastr['error']('At Least One Product Needed For Adding Order');
    }
    else {
      if (salesType === 'Cash') {
        createOrUpdateCreditSale(event.target, productsArray, vertical, grandTotal,
          taxTotal, latitude, longitude, outlet, salesType, walkInCustomer);
        FlowRouter.go('creditSale');
        dataClear();
        function dataClear() {
        ...
        if (Number(grandTotal) <= Number(creditAmt)) {
          createOrUpdateCreditSale(event.target, productsArray, vertical, grandTotal,
            taxTotal, latitude, longitude, outlet, salesType, walkInCustomer);
          FlowRouter.go('creditSale');
          dataClear();
...
*/
    ...
    if (unit !== undefined && unit !== '' && vertical !== '' && vertical !== undefined && product !== '' && product !== undefined) {
      ...
      Meteor.call('unit.priceCal', product, unit, vertical, Meteor.userId(), channelval, (err, res) => {
        if (!err) {
          ...
          $('#priceVals').val(Number(res.productPrice));
  ...
  */
   ...
   * @param {*} template 
   */
  'change #selectOutlet': (event, template) => {
    event.preventDefault();
    let outlet = '';
    $('#selectOutlet').find(':selected').each(function () {
      outlet = $(this).val();
    ...
    $('#creditLtdVal').html('');
    $('#creditPeriodVal').html('');
    template.modalLoader.set(false);
    walkInCustomer = false;
    template.creditAmtVal.set('');
    $('#salesType').prop('disabled', false);
    if (outlet !== '' && outlet !== undefined && outlet !== 'Walk-In Customer') {
      template.modalLoader.set(true);
      walkInCustomer = false;
      Meteor.call("outlet.idDatas", outlet, (err, res) => {
        if (!err) {
          $('#creditLtdVal').html(`Credit Limit : ${res.creditAmount}`);
          $('#creditPeriodVal').html(`Credit Period : ${res.creditPeriod}`);
          template.modalLoader.set(false);
          template.creditAmtVal.set(res.creditAmount);
        }
        else {
          $('#creditLtdVal').html('');
          $('#creditPeriodVal').html('');
    ...
    else if (outlet === 'Walk-In Customer') {
      $('#salesType').val('Cash').trigger('change');
...
*/
  'change #selectCategory': (event, template) => {
    ...
    if (category !== undefined && category !== '' && brand !== undefined && brand !== '' &&
      ...
      template.modalLoader.set(true);
      Meteor.call('wareHouseStock.productList', vertical, Meteor.userId(), brand, category, principal, (err, res) => {
        if (!err) {
          template.productListGet.set(res);
          template.modalLoader.set(false);
        }
