...
Template.creditSale.onCreated(function () {
  ...
  this.saleIdListVar = new ReactiveVar();
...
Template.creditSale.onRendered(function () {
 ...
 */
   ...
   * get product list
   */
  Meteor.call('creditSale.SaleIdList', Meteor.userId(), (err, res) => {
    if (!err) {
      this.saleIdListVar.set(res);
    }
  });
  ...
  $('.saleType').select2({
    ...
    tokenSeparators: [','],
    allowClear: true,
    dropdownParent: $(".saleType").parent(),
  });

});

Template.creditSale.helpers({
  /**
               * TODO: Complete JS doc
               * @returns {any | *}
               */
 ...
 */
  ...
  saleIdList1: function () {
    return Template.instance().saleIdListVar.get();
  },
  /**
     * TODO: Complete JS doc
     * @returns {*}
...
Template.creditSale.events({
  ...
  'submit .stockFilter': (event) => {
    ...
    let saleId = event.target.saleId1.value;
    let saleType = event.target.saleType.value;
    ...
    if(saleId && saleType==='' && fromDate && toDate){
    ...
    }else if(saleId==='' && saleType && fromDate && toDate){
      Template.instance().pagination.settings.set('filters', {
          salesType:saleType,
          createdAt: {
            $gte: fromDate, $lt: toDate
  ...
  'click #ic-create-stock-button': () => {
    ...
    if (channelval === "VSR") {
      Meteor.call("routeAssign.checkRoute", Meteor.userId(), (err, res) => {
        if (!err) {
          if (res.length > 0) {
            FlowRouter.go('createCreditSale');
          }
          else {
  ...
  'click .view': (event, template) => {
    ...
    let detailSalesType = $('#detailSalesType');
    ...
    Meteor.call('creditSale.id', id, (err, res) => {
      console.log("res ",res);
      if (!err) {
        template.modalLoader.set(false);
        $(header).html('Details of ' + $.trim(res.creditSaleData.docNum));
        $(saleId).html(res.creditSaleData.docNum);
        $(detailvertical).html(res.verticalName);
        $(detailDate).html(res.creditSaleData.docDate);
        $(detailRoute).html(res.routeIdName);
        $(detailOutletName).html(res.outletName);
        $(detailEmp).html(res.sdName);
        $(detailSalesType).html(res.creditSaleData.salesType);
        $(detailGSTs).html(res.creditSaleData.taxAmount.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
        $(detailDocTotals).html(res.creditSaleData.docTotal.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
        if (res.creditSaleData.itemArray !== undefined && res.creditSaleData.itemArray.length > 0) {
          template.itemsDetailsList.set(res.creditSaleData.itemArray);
        }
      }
    });
