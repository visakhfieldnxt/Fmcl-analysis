...
Template.sdUserStockList.onCreated(function () {
...
Template.sdUserStockList.onRendered(function () {
...
Template.sdUserStockList.helpers({
  /**
               * TODO: Complete JS doc
               * @returns {any | *}
               */
...
*/
  totalValueGet: (_id) => {
    let promiseVal = new Promise((resolve, reject) => {
      Meteor.call("wareHouseStock.idPriceValGet", _id, (error, result) => {
        if (!error) {
          resolve(result);
        } else {
          reject(error);
    ...
    promiseVal.then((result) => {
      $('.stockPriceVal_' + _id).html(`${result} GHS`);
...
Template.sdUserStockList.events({
  ...
  'submit .stockFilter': (event) => {
    ...
    if (vertical) {
      Template.instance().pagination.settings.set('filters',
        {
          vertical: vertical, employeeId: Meteor.userId(),
          stock: { $ne: "0.00" }
        }
      );
