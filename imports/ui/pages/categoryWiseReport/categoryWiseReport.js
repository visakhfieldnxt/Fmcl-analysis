...
Template.categoryWiseReport.onCreated(function () {
  let self = this;
  self.autorun(() => {
...
Template.categoryWiseReport.onRendered(function () {
...
Template.categoryWiseReport.helpers({
  ...
  salesVolume: (id) => {
    ...
    let promiseVal = new Promise((resolve, reject) => {
      Meteor.call('creditSale.totalSalesCategoryVolume', Meteor.userId(), id, fromDate, toDate, (error, result) => {
        if (!error) {
  ...
  salesValue: (id) => {
    ...
    let promiseVal = new Promise((resolve, reject) => {
      Meteor.call('creditSale.totalSalesCategoryValue', Meteor.userId(), id, fromDate, toDate, (error, result) => { // totalSalesCategoryWise
        if (!error) {
  ...
  totalVolume: () => {
    ...
    let promiseVal = new Promise((resolve, reject) => {
      Meteor.call('creditSale.totalSalesCatVlmTotal', Meteor.userId(), fromDate, toDate, category, (error, result) => { // totalSalesCategoryWise
        if (!error) {
          resolve(result.roundCtn + ' CTN & ' + result.findPcs + ' PCS');
        } else {
          reject(error);
  ...
  gtotalValue: () => {
    ...
    let category = Template.instance().category.get();
    let promiseVal = new Promise((resolve, reject) => {
      Meteor.call('creditSale.totalSalesCatVleTotal', Meteor.userId(), fromDate, toDate, category, (error, result) => { // totalSalesCategoryWise
        if (!error) {
          resolve(result.toFixed(2));
        } else {
          reject(error);
...
Template.categoryWiseReport.events({
  'click #filterSearch': () => {
