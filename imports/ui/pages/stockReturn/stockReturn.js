...
Template.stockReturn.onCreated(function () {
...
Template.stockReturn.onRendered(function () {
 ...
 */
  ...
  if (Meteor.user()) {
    ...
    Meteor.call('stockReturn.idList', Meteor.userId(), (err, res) => {
      if (!err) {
...
Template.stockReturn.events({
  ...
  'submit .stockFilter': (event) => {
    ...
    if (vertical && uniqueId === '') {
      Template.instance().pagination.settings.set('filters',
        {
          vertical: vertical, createdBy: Meteor.userId(),
        }
      );
      ...
      Template.instance().pagination.settings.set('filters',
        {
          temporaryId: uniqueId, createdBy: Meteor.userId(),
        }
      );
      $('.taskHeaderList').css('display', 'none');
    }
  ...
  'click #ic-create-stock-button': () => {
    FlowRouter.go('createStockReturn');
  },
  ...
  'click .view': (event, template) => {
    ...
    FlowRouter.go('stockReturnDetails', { _id: id });
