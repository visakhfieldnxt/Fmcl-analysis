...
Template.regionWiseSalesReport.onCreated(function () {
  let self = this;
  self.autorun(() => {
...
Template.regionWiseSalesReport.onRendered(function () {
...
Template.regionWiseSalesReport.helpers({
  ...
  bnLists: () => {
    return Template.instance().listBranch.get();
  }
});


Template.regionWiseSalesReport.events({
  'click #filterSearch': () => {
  ...
  'click #subIdSubmit': (event, template) => {
    ...
    // if (verticalName == '') {
    //   setTimeout(function () {
    //     $("#itemArrayspan1").html('<style> #itemArrayspan1 { color:#fc5f5f }</style><span id ="itemArrayspan">Please Select Vertical</span>').fadeIn('fast');
    //   }, 0);
    //   return;
    // } else {
    //   $("#itemArrayspan1").html('');
    ...
    //   template.verticalVar.set(verticalName);
    //   Template.instance().pagination.settings.set('filters', {
    //     active: "Y", vertical: verticalName
    //   });
    // } else {
    //   Template.instance().pagination.settings.set('filters', {
    //     active: "Y"
  ...
  'change .verticalfilter': (event, template) => {
    ...
    Meteor.call('branch.withRegionVerical', vertical, (err, res) => {
      if (!err) {
        template.regionArray.set(res);
      }
    });
  },
  ...
  'change .endDate1,#verticalfilters,.startDate1': (event, template) => {
  ...
  'click .exportToday': (event, template) => {
    ...
    let exportValue = template.listBranch.get();
