...
Template.approvedSdUsers.onCreated(function () {
...
Template.approvedSdUsers.onRendered(function () {
...
Template.approvedSdUsers.helpers({
...
Template.approvedSdUsers.events({
  /**
   * TODO: Complete JS doc
   * @param event
   */
      ...
      Template.instance().pagination.settings.set('filters',
        {
          ...
          userType: "SDUser",
          active: "Y", approvalStatus: "Approved"
        }
      );
    }
    else {
  ...
  'click .reset': () => {
    Template.instance().pagination.settings.set('filters', {
      active: "Y",
      userType: "SDUser", approvalStatus: "Approved"
    });
    $('form :input').val("");
  ...
  'click .view': (event, template) => {
    ...
    let detailApprovedRemarks = $('#detailApprovedRemarks');
    ...
    Meteor.call('user.sdUserData', id, (userError, userResult) => {
      if (!userError) {
        ...
        $(detailApprovedBy).html(userResult.approvedByName);
        $(detailApprovedDate).html(moment(user.approvedDate).format('DD-MM-YYYY hh:mm A'));
        $(detailApprovedRemarks).html(user.approvalRemark);
      }
    });
