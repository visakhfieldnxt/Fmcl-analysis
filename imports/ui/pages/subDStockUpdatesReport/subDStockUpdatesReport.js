...
Template.subDStockUpdatesReport.onCreated(function () {
...
Template.subDStockUpdatesReport.onRendered(function () {
...
Template.subDStockUpdatesReport.helpers({
  /**
               * TODO: Complete JS doc
               * @returns {any | *}
               */
...
Template.subDStockUpdatesReport.events({
  ...
  'submit .stockFilter': (event, template) => {
    ...
    if (vertical && product === '' && isNaN(fromDate) && isNaN(toDate)) {
      Template.instance().pagination.settings.set('filters', {
        vertical: vertical, subDistributor: Meteor.userId(),
      });
    }
    else if (vertical === '' && product && isNaN(fromDate) && isNaN(toDate)) {
      Template.instance().pagination.settings.set('filters', {
        product: product, subDistributor: Meteor.userId(),
      });
    }
    else if (vertical && product && isNaN(fromDate) && isNaN(toDate)) {
      Template.instance().pagination.settings.set('filters', {
    ...
    else if (fromDate && isNaN(toDate) && vertical === '' && product === '') {
    ...
    else if (toDate && isNaN(fromDate) && vertical === '' && product === '') {
    ...
    else if (fromDate && toDate && vertical === '' && product === '') {
    ...
    else if (vertical && toDate && fromDate && product) {
      if (fromDate.toString() === toDate.toString()) {
        toDate.setDate(toDate.getDate() + 1);
        Template.instance().pagination.settings.set('filters', {
          createdAt: {
...
*/
  ...
  */
  ...
  'change .endDate1': (event, template) => {
    ...
    template.stockExportData.set('');
    template.modalLoader.set(true);
    Meteor.call('stockInvoices.exportData', vertical, product, Meteor.userId(), fromDate, toDates, (err, res) => {
      if (!err) {
        template.modalLoader.set(false);
        if (res.length === 0) {
          setTimeout(function () {
