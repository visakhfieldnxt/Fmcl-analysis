...
Template.stockReturnAccept.onCreated(function () {
...
Template.stockReturnAccept.onRendered(function () {
 ...
 */
  ...
  if (Meteor.user()) {
    ...
    Meteor.call('stockReturn.SdList', Meteor.userId(), (err, res) => {
      if (!err) {
...
Template.stockReturnAccept.helpers({
  /**
               * TODO: Complete JS doc
               * @returns {any | *}
               */
...
Template.stockReturnAccept.events({
  ...
  'submit .stockFilter': (event) => {
    ...
    if (vertical && uniqueId === '') {
      ...
      Template.instance().pagination.settings.set('filters',
        {
          temporaryId: uniqueId, subDistributor: Meteor.userId(),
          status: "Pending"
        }
      );
      $('.taskHeaderList').css('display', 'none');
  ...
  'click .view': (event, template) => {
    ...
    FlowRouter.go('stockReturnAcceptDetails', { _id: id });
    // template.modalLoader.set(true);
    // let header = $('#stockH');
