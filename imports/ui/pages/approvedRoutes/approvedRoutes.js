...
Template.approvedRoutes.onCreated(function () {
  const self = this;
  self.autorun(() => {
  });
...
Template.approvedRoutes.helpers({
  /**
   * TODO: Complete JS doc
   * @returns {any | *}
   */
...
Template.approvedRoutes.events({
  /**
   * TODO: Complete JS doc
   * @param event
   */
  ...
  'click .view': (event, template) => {
    ...
    Meteor.call('routeAssign.id', id, (err, res) => {
      if (!err) {
        ...
        $(detailApprovedName).html(res.approvedName);
        $(detailApprovedDate).html(moment(res.routeAssignRes.approvedDate).format('DD-MM-YYYY hh:mm A'));
